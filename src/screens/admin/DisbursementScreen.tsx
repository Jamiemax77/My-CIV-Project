...
const transferSchema = z
  ...
  .refine(
    data =>
      data.method === 'tunai' || (data.senderBank?.trim().length ?? 0) >= 2,
    {
  ...
  .refine(
    data =>
      data.method === 'tunai' || (data.destAccount?.trim().length ?? 0) >= 4,
    {
...
export function DisbursementScreen () {
  ...
  const [lastTransferProof, setLastTransferProof] =
    useState<LastTransferProof | null>(null)
  ...
  useEffect(() => {
    if (
      !dataForm.getValues('participantId') &&
      participants &&
  ...
  useEffect(() => {
    ...
    transferForm.setValue(
      'destAccount',
      accounts.length === 1 ? formatAccountLabel(accounts[0]) : ''
    )
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const submitData = async (values: DataFormValues, advance: boolean) => {
    const participant = participants?.find(
      p => p.profile.id === values.participantId
    )?.profile
    setSubmitError(null)
    ...
    try {
      ...
      setSubmitError(
        err instanceof Error ? err.message : 'Gagal menyimpan pencairan.'
      )
    }
  }
  ...
  const continueExisting = () => {
    ...
    const participant = participants?.find(
      p => p.profile.id === existingParticipantId
    )?.profile
    if (otherTransaction) {
    ...
    const picked = (existingDisbursements ?? []).find(
      d => d.id === selectedDisbursementId
    )
    if (!picked) {
      setExistingError(
        'Pilih pencairan, atau isi keterangan untuk transaksi lainnya'
      )
      return
    }
    setActiveTarget({
    ...
    try {
      ...
      await addTransferProof.mutateAsync({
        ...
        senderBank:
          values.method === 'tunai'
            ? TUNAI_SENDER_BANK
            : (values.senderBank ?? '').trim(),
        destAccount:
          values.method === 'tunai'
            ? TUNAI_DEST_ACCOUNT
            : (values.destAccount ?? '').trim(),
      ...
      setSavedMessage(
        `Bukti transfer terkirim ke ${activeTarget.participantName}.`
      )
  ...
  const exportVoucher = async (share: boolean) => {
    ...
    try {
      ...
      setExportError(
        err instanceof Error ? err.message : 'Gagal membuat PDF voucher.'
      )
    } finally {
  ...
  const exportTransferProof = async (share: boolean) => {
    ...
    try {
      ...
      setExportError(
        err instanceof Error ? err.message : 'Gagal membuat PDF bukti transfer.'
      )
    } finally {
        ...
        <ResponsiveContainer>
          {participantsLoading ? (
                ...
                <View style={styles.gap}>
                  ...
                  <View style={styles.gap}>
                    ...
                    {dataForm.formState.errors.program ? (
                      <Text style={styles.errorText}>
                        {dataForm.formState.errors.program.message}
                      </Text>
                    ) : null}
                  </View>
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <Field
                          ...
                          value={field.value}
                          onChangeText={text =>
                            field.onChange(formatAmountInput(text))
                          }
                  ...
                  {lastTransferProof ? (
                    <View style={styles.btnRow}>
                      <Button
                        label={
                          transferPdfExporting
                            ? 'Membuat PDF...'
                            : 'Export Bukti Transfer'
                        }
                        variant='ghost'
                        style={styles.btn}
                  ...
                  <View style={styles.btnRow}>
                    <Button
                      ...
                      onPress={dataForm.handleSubmit(values =>
                        submitData(values, false)
                      )}
                    />
                    <Button
                      label='Proses Bukti Transfer'
                      variant='navy'
                      style={styles.btn}
                      disabled={addDisbursement.isPending}
                  ...
                  <View style={styles.gap}>
                    ...
                    <ChipGroup>
                      {(existingDisbursements ?? []).map(d => (
                        <Chip
                          ...
                          active={
                            !otherTransaction && selectedDisbursementId === d.id
                          }
                          onPress={() => {
                            setOtherTransaction(false)
                    ...
                    {(existingDisbursements ?? []).length === 0 ? (
                      <Text style={styles.hintText}>
                        Tidak ada pencairan yang menunggu bukti transfer untuk
                        partisipan ini.
                      </Text>
                    ) : null}
                  </View>
            ...
            <>
              ...
              <Pressable
                onPress={
                  mode === 'existing' ? () => setStep('data') : undefined
                }
              ...
              >
                ...
                {!isOther ? (
                  <Text style={styles.targetAmount}>
                    {formatRupiah(activeTarget?.amount ?? 0)}
                  </Text>
                ) : null}
                {mode === 'existing' ? (
              ...
              {!isOther ? (
                <View style={styles.btnRow}>
                  <Button
                    label={
                      voucherExporting ? 'Membuat PDF...' : 'Export Voucher'
                    }
                    variant='ghost'
                    style={styles.btn}
              ...
              {method === 'transfer' ? (
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <Field
                          ...
                          onChangeText={field.onChange}
                          error={
                            transferForm.formState.errors.senderBank?.message
                          }
                        />
                      )}
                    />
                  ...
                  <View style={styles.gap}>
                    {!accounts ? null : accounts.length === 0 ? (
                      <Text style={styles.warningText}>
                        ⚠ Partisipan ini belum mengisi rekening bank atau
                        e-wallet. Isi rekening tujuan secara manual di bawah,
                        atau minta partisipan melengkapinya dulu di menu
                        Rekening & E-Wallet.
                      </Text>
                    ) : (
                      <ChipGroup>
                        {accounts.map(a => (
                          <Chip
                            ...
                            active={
                              transferForm.watch('destAccount') ===
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <Field
                          ...
                          onChangeText={field.onChange}
                          error={
                            transferForm.formState.errors.destAccount?.message
                          }
                          style={
                ...
                <View style={styles.gap}>
                  ...
                  <Text style={styles.hintText}>
                    Partisipan menerima dana secara tunai langsung dari
                    bendahara — bank pengirim dan rekening tujuan tidak
                    diperlukan.
                  </Text>
                </View>
              )}
              ...
              <Text style={styles.fieldLabel}>
                {method === 'tunai'
                  ? 'Lampiran Bukti Tanda Terima Tunai'
              ...
              <View style={styles.btnRow}>
                ...
                <Button
                  label={
                    submittingTransfer ? 'Mengirim...' : 'Kirim ke Partisipan'
                  }
                  variant='navy'
                  style={styles.btn}
