...
import {
  ...
  JenisPengajuanKode
} from '../../lib/generateNomorPengajuan'
import { REIMBURSEMENT_CATEGORY_LABEL } from '../../lib/labels'
import {
  FormPengajuan,
  FormPengajuanSubmitValues
} from './reimbursement/FormPengajuan'
import { KasBonPengajuan } from './reimbursement/KasBonPengajuan'
...
export function ReimbursementScreen () {
  ...
  const [previewTarget, setPreviewTarget] = useState<ReimbursementItem | null>(
    null
  )
  ...
  const onSubmitForm = async (values: FormPengajuanSubmitValues) => {
    ...
    try {
      ...
      const uploadedUsageProof = await uploadFile(
        values.usageProof,
        'reimbursement',
        token,
        user.id
      ...
      await addReimbursement.mutateAsync({
        type: wizard.jenis === 'return' ? 'return' : 'reimburse',
      ...
      setSuccessMessage(
        `Pengajuan ${wizard.nomorPengajuan} terkirim, menunggu verifikasi admin.`
      )
      dispatch({ type: 'RESET' })
    } catch (err) {
      setSubmitError(
  ...
  return (
    ...
    >
      <Header title='Ajukan Pengembalian / Dana' />
      <ScrollView
            ...
            <>
              <StepPemilihan
                ...
                onSelectKategori={kategori =>
                  dispatch({ type: 'SET_KATEGORI', kategori })
                }
              ...
              <Text style={[styles.fieldLabel, styles.historyTitle]}>
                Riwayat Pengajuan
              </Text>
              {historyLoading ? (
                ...
                history.map(item => (
                  <ListItem
                    ...
                    title={`${
                      ...
                      REIMBURSEMENT_CATEGORY_LABEL[item.category]
